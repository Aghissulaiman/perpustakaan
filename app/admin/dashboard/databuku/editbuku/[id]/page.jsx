...
export default function EditBukuPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await updateBook(new FormData(e.target));
      if (result.success) {
        alert("📚 Buku berhasil diperbarui!");
        router.push("/admin/dashboard/databuku");
      } else {
        alert("Gagal memperbarui buku: " + result.error);
      }
    } catch (error) {
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-xl border border-gray-200 p-10">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputField label="ISBN" name="isbn" value={formData.isbn} onChange={handleChange} placeholder="978-0-123456-78-9" />
            </div>
